...
for filename in glob.iglob("**/*.ipynb", recursive=True):
    ...
    for line in open(filename, "r"):
        if re.search("# Skip notebook test", line):
            ...
            print(
                f"SKIPPING {filename} (suspected Dask usage, not currently automatable)",
                file=sys.stderr,
            )
            skip = True
            break
            ...
            print(
                f"SKIPPING {filename} (does not run on Pascal)",
                file=sys.stderr,
            )
            skip = True
            break
        elif re.search("CVRPTW Exercise", line):
            print(
                f"SKIPPING {filename} (user exercise notebook)",
                file=sys.stderr,
            )
            skip = True
            break

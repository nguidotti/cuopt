...
if __name__ == "__main__":
    ...
    if not isinstance(procs_per_gpu, int):
        raise ValueError(
            f"Process per GPU should be an integer value, current value is of type : {type(procs_per_gpu)}"  # noqa: E501
        )
    ...
    parser.add_argument(
        "-i", "--ip", type=str, help="IP to the server", default=ip
    )
    parser.add_argument(
        "-p", "--port", type=int, help="Port of the server", default=port
    )
    parser.add_argument(
        "-d",
        "--datadir",
        type=str,
        help="Data directory used to optionally pass cuopt "
        "problem data files to /cuopt/request endpoint via the filesystem "
    ...
    parser.add_argument(
        ...
        "--resultdir",
        type=str,
        help="Output directory used to optionally pass cuopt "
        "result files from /cuopt/request endpoint via the filesystem "
        "instead of over http",
        default=resultdir,
    )
    parser.add_argument(
        ...
        "--maxresult",
        type=int,
        help="Maximum size (kilobytes) of a result returned over http from "
        "/cuopt/request endpoint when RESULTDIR is set. Set to 0 to have "
        "all results written to RESULTDIR (default 250)",
        default=maxresult,
    )
    parser.add_argument(
        "-mo",
        "--mode",
        type=str,
        help="Mode (octal) of result files from "
        "/cuopt/request endpoint (default is 644)",
        default=resultmode,
    )
    parser.add_argument(
        "-l",
    ...
    parser.add_argument(
        ...
        "--log-file",
        type=str,
        help="Log filename (by default logs to stdout)",
        default=log_file,
    )
    parser.add_argument(
        "-lm",
    ...
    parser.add_argument(
        "--ssl-certfile",
        type=str,
        help="SSL certificate file with complete path.",
        default=ssl_certfile,
    )
    parser.add_argument(
        "--ssl-keyfile",
        type=str,
        help="SSL key file with complete path.",
        default=ssl_keyfile,
    )
    parser.add_argument(
        "-g",
        "--gpu-count",
        type=int,
        help="Number of available GPUs to use for solver processes "
    ...
    parser.add_argument(
        ...
        help="Feature tier for /cuopt/cuopt endpoint. This names a feature "
        "tier defined in request_filter.py which restricts allowed features "
        "or values for datasets in a request. Not currently implemented for "
        "the /cuopt/request endpoint. See request_filter.py ",
        default=tier,
    )
    ...
    if args.log_file:
        if args.log_backup < 1:
            parser.error("Minimum number of backup log files is 1")
        handlers = [
